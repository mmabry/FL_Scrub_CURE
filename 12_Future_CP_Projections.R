...
input_base_dir <- "06_rasters/CoastalPlain/Future/"  # Base directory for rasters
...
for (i in seq_along(unique_species)) {
  if (i < 24) next  # Skip the first 23 species
  ...
  for (model_dir in model_dirs) {
    ...
    Soil <- list.files("06_rasters/CoastalPlain/Current", pattern = "*v2.asc", full.names = TRUE)
    Elev <- list.files("06_rasters/CoastalPlain/Current", pattern = "*elev.asc", full.names = TRUE)
    ...
    projection_filename <- file.path(output_base_dir, paste0(spec, "_", model, "_", time_period, "_", ssp, "_ENM_CP_Projection.asc"))
    ...
    save(p, file = file.path(output_base_dir, paste0(spec, "_", model, "_", time_period, "_", ssp, "_ENM_CP_Projection.RData")))
    ...
    # Plot and save
    plot_filename <- file.path(output_base_dir, paste0(spec, "_", model, "_", time_period, "_", ssp, "_ENM_CP_Projection.pdf"))
